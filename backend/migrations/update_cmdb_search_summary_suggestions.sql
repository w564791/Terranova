-- 强化 suggestions 规则：必须为可直接回填搜索框的纯查询词
-- 与代码 defaultCMDBSearchSummaryPrompt 规则 10 对齐；幂等覆盖 capability_prompts
...
10. suggestions 必须是可直接填入搜索框的「纯查询词」，禁止说明/引导前缀
   - 正确: "test-ken-manifest policy"、"s3 public"、"prod ec2 ap-northeast-1"
   - 错误: "可查询存储桶策略详情：test-ken-manifest policy"、"建议搜索 xxx"、"试试：yyy"
   - 每条不超过 40 字，不要用冒号拼接说明
