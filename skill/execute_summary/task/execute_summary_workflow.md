...
4. **评估风险等级**:
   - **low**: 仅新增资源且无安全风险，无依赖影响
   - **medium**: 修改资源但无破坏性变更，或有少量依赖方，或新增非敏感端口的内网安全组规则
   - **high**: 删除资源且有依赖方，或收窄安全组端口范围（可能导致现有服务中断），或修改安全相关配置
   - **critical**: 删除核心网络资源（VPC/子网）、大规模删除、修改 IAM 权限，或以下安全组场景

   **安全组变更专项判定（优先级高于通用规则）:**
   - 任何规则的 CIDR 包含 `0.0.0.0/0` 或 `::/0`（面向全公网）:
     - 端口为数据库端口（3306/5432/1521/27017/6379/9200）→ **critical**（数据库暴露公网）
     - 端口为管理端口（22/3389）→ **critical**（远程管理暴露公网）
     - 端口为全端口（-1 或 0-65535）→ **critical**（全端口暴露公网）
     - 端口为 Web 端口（80/443/8080/8443）→ **medium**（可能是正常业务）
     - 其他端口 → **high**（非标准端口暴露公网需关注）
   - 安全组 inbound 端口范围收窄（如从 -1 到特定端口）:
     - 需查询 CMDB 依赖方确认是否影响现有服务，有依赖方且端口不匹配 → **critical**
   - 安全组被删除且有依赖方 → **critical**
