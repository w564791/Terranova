...
## 模式 9：避免循环依赖的 IAM Role 授权

在 Terraform 场景中授权 IAM Role 访问 S3 桶时，使用 `Principal: "*"` + `aws:PrincipalArn` Condition 替代直接 Principal，避免循环依赖。

详见 `aws_policy_core_principles` 的"Terraform 场景：避免循环依赖"章节。
